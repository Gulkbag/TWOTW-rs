...
const TAZER_DAMAGE: i32 = 40;
const TAZER_RANGE: i32 = 5;
const CONFUSE_RANGE: i32 = 8;
const CONFUSE_NUM_TURNS: i32 = 10;
const COOKER_RADIUS: i32 = 3;
const COOKER_DAMAGE: i32 = 25;
...
fn cast_heal(
    ...
    if let Some(fighter) = player.fighter {
        ...
        game.log.add(
            "The pizza fills your up with shit, and heals your wounds.",
            colors::LIGHT_VIOLET,
        );
...
fn taze(
    ...
    let monster_id = closest_monster(TAZER_RANGE, objects, tcod);
    if let Some(monster_id) = monster_id {
        ...
        game.log.add(
            format!(
                ...
                objects[monster_id].name, TAZER_DAMAGE
            ),
            colors::LIGHT_BLUE,
        );
        if let Some(xp) = objects[monster_id].take_damage(TAZER_DAMAGE, game) {
...
fn cast_cooker(
    ...
    // ask the player for a target tile to throw a cooker at
    game.log.add(
        "Left-click a target tile for the cooker, or right-click to cancel.",
        colors::LIGHT_CYAN,
    );
    ...
    game.log.add(
        format!(
            "The cooker explodes, burning everything within {} tiles!",
            COOKER_RADIUS
        ),
        colors::ORANGE,
    );
    ...
    for (id, obj) in objects.iter_mut().enumerate() {
        if obj.distance(x, y) <= COOKER_RADIUS as f32 && obj.fighter.is_some() {
            game.log.add(
                format!(
                    "The {} gets burned for {} hit points.",
                    obj.name, COOKER_DAMAGE
                ),
                colors::ORANGE,
            );
            if let Some(xp) = obj.take_damage(COOKER_DAMAGE, game) {
                if id != PLAYER {
        ...
        if !is_blocked(x, y, map, objects) {
            let mut monster = match monster_choice.ind_sample(&mut rand::thread_rng()) {
                "nigger" => {
                    // create a nigger
                    let mut nigger =
                        Object::new(x, y, 'O', "Nigger", colors::DESATURATED_GREEN, true);
                    nigger.fighter = Some(Fighter {
                        base_max_hp: 20,
                        hp: 20,
                        base_defense: 0,
                        base_power: 2,
                    ...
                    nigger.ai = Some(Ai::Basic);
                    nigger
                }
                "jew" => {
                    // create a jew
                    let mut jew = Object::new(x, y, 'T', "Jew", colors::DARKER_GREEN, true);
                    jew.fighter = Some(Fighter {
                        base_max_hp: 30,
                        hp: 30,
                        base_defense: 2,
                        base_power: 8,
    ...
    for _ in 0..num_items {
        ...
        if !is_blocked(x, y, map, objects) {
            let mut item = match item_choice.ind_sample(&mut rand::thread_rng()) {
                Item::Heal => {
                    // create a healing potion-like item
                    let mut object = Object::new(x, y, 'V', "Jew Pizza", colors::PEACH, false);
                    object.item = Some(Item::Heal);
                    object
                }
                Item::Tazer => {
                    // create a lightning bolt-like item
                    let mut object = Object::new(x, y, '#', "Tazer", colors::LIGHT_YELLOW, false);
                    object.item = Some(Item::Tazer);
                    object
                }
                Item::Cooker => {
                    // create a fireball scroll-like item
                    let mut object =
                        Object::new(x, y, 'E', "Jew Cooker", colors::LIGHT_YELLOW, false);
                    object.item = Some(Item::Cooker);
                    object
                }
                Item::Confuse => {
                    // create a confusion-scroll like item
                    let mut object =
                        Object::new(x, y, 'B', "KFC/Shekel Bag", colors::LIGHT_YELLOW, false);
                    object.item = Some(Item::Confuse);
                    object
                }
                Item::Sword => {
                    // create a sword-like item
                    let mut object = Object::new(x, y, '/', "Bosniak Slayer", colors::SKY, false);
                ...
                Item::Shield => {
                    // create a sheild-like item
                    let mut object = Object::new(
                        x,
                        y,
                        '[',
                        "Nigger Sheild (A Nigger's Body)",
...
fn next_level(tcod: &mut Tcod, objects: &mut Vec<Object>, game: &mut Game) {
    ...
    game.log.add(
        "After a rare moment of not seeing nigger faces, you descend deeper into \
...
fn level_up(objects: &mut [Object], game: &mut Game, tcod: &mut Tcod) {
    ...
    if player.fighter.as_ref().map_or(0, |f| f.xp) >= level_up_xp {
        ...
        game.log.add(
            format!(
                "Your nigger killing skills grow stronger! You reached level {}!",
                player.level
            ),
            colors::YELLOW,
        );
...
fn new_game(tcod: &mut Tcod) -> (Vec<Object>, Game) {
    ...
    game.log.add(
        "Your upper class KKK members dropped you into a minority dungeon, Hope you know how to kill niggers.",
        colors::RED,
    );
    game.log.add(
        "You found a kebab stick, you pick it up to help you with fightning",
        colors::BLUE,
    );
